...
IF codtall > '010'
     ...
     RETURN .F.
ENDIF
IF auxest = '021 ' .OR. auxest =  ;
   '026 '
     DO error WITH  ;
        '*** La O/R se encuentra en Almacen sin reparar ***'
     USE
     RETURN .F.
ENDIF
IF SUBSTR(indori, 2, 1) = 'R'
     DO error WITH  ;
        '*** N§ Orden Reparaci¢n es Reclamo ***'
     USE
     RETURN .F.
ENDIF
IF indori = 'FGAR'
...
IF auxest = '080 '
     USE
     SELECT 20
     USE st_mvord ORDER ordia
     SEEK wk_clave + '021 '
     IF FOUND()
          DO error WITH  ;
             '*** N§ O/R ha pasado por almac‚n sin Reparar ***'
          USE
          RETURN .F.
     ENDIF
ENDIF
...
USE
@ 10, 051 SAY wrk_desmar
@ 10, 076 SAY wk_numser
IF wk_ogar = 'GARA'
