...
***
titu1 = ' MANTENCION '
titu2 = ' SALIDA DE ORDEN SUCURSALES '
wrk_progra = PROGRAM()
CLOSE DATABASES
DO crea_win
...
IF SUBSTR(indori, 1, 1) <> 'F'
     USE
     DO error WITH  ;
        '** N§ Orden Reparaci¢n no es Fuera de Garant¡a. **'
     KEYBOARD '{CTRL+Y}' PLAIN
     RETURN .F.
ENDIF
...
IF a > 0
     USE
     DO error WITH  ;
        '** Error tiene repuestos cargados **'
     RETURN .F.
ENDIF
USE
RETURN .T.
