...
DO saycenter WITH 2,  ;
   'INFORME ORDENES POR FACTURAR '
tipri = ' INFORME ORDENES POR FACTURAR '
@ 3, 2 TO 5, 77
ppas = .T.
SELECT 1
USE SHARED st_IOREP ORDER CODIGO
pas496 = da_nombre()
Create table &PAS496 (pas_numsol c(8),pas_numord;
C(8),pas_fecemi D(8),pas_fecfin d(8),pas_cli;
c(9),pas_mode C(15),pas_desmod c(25),pas_serie;
c(20),pas_total n(10))
SELECT 2
use &PAS496 EXCLUSIVE      
DO WHILE ppas
     @ 7, 1 CLEAR TO 13, 77
     @ 4, 30 SAY SPACE(30)
     @ 3, 2 TO 5, 77
     ...
     READ
     SET CURSOR OFF
     pregunta = 'fecfin<=wk_fec.and.indest="C"'
     ...
     IF wk_hay = .F.
          DO error WITH  ;
             '** No Se Registra Ninguna Orden Por Facturar **'
          LOOP
     ENDIF
     key = INKEY()
...
IF sw_impre <> 1
     RETURN
ENDIF
tit1 = 'No.SOLIC. No. ORDEN FECHA DE  FECHA DE  CLIENTE        SERIE          VALOR     '
tit2 = '                    ORDEN     REPARAC.   '
con_lin = 12
DO esc_modo WITH 'P'
SET PRINTER ON
