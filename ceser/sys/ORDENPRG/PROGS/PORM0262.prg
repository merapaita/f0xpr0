...
DO saycenter WITH 2,  ;
   ' Facturas de Preventa y/o Postventa '
a = 1
...
DO WHILE ppas
     ...
     IF sw_algo = .T.
          ...
          ELSE
               @ 04, 20 SAY  ;
                 'Documento de Venta...:  FACT O LIQU'
          ENDIF
          @ 06, 20 SAY  ;
            'Fecha de la Factura..:'
          @ 07, 20 SAY  ;
               ...
               ELSE
                    DO CASE
                         CASE w_estado =  ;
                              ...
                              IF w_coddoc =  ;
                                 'LIQU'
                                   REPLACE auxest WITH '081 '
                              ELSE
                                   REPLACE auxest WITH '080 '
                              ENDIF
                         CASE w_estado =  ;
                              '020 '  ;
                              .OR.  ;
...
REPLACE hve_cosrep WITH w_repues
REPLACE hve_totnet WITH w_totnet
...
REPLACE hve_pagctd WITH ctatot
REPLACE hve_usuari WITH users
REPLACE hve_fecha WITH DATE()
REPLACE hve_hora WITH TIME()
REPLACE hve_codemi WITH w_codemi
REPLACE hve_solrep WITH s_repues
REPLACE hve_solmob WITH s_mano
REPLACE hve_solfle WITH  ;
        ROUND(s_flete * (1 +  ;
        w_facigv), 2)
REPLACE hve_solnet WITH s_totnet
REPLACE hve_soldes WITH s_descue
REPLACE hve_solvta WITH s_totvta
REPLACE hve_soligv WITH s_totigv
REPLACE hve_solgen WITH s_totgen
REPLACE hve_tipcam WITH w_tipcam
REPLACE hve_mtocan WITH s_totbru
REPLACE hve_pagcts WITH s_ctatot
UNLOCK
SELECT 20
...
ELSE
     ...
     IF w_codmon = 'DOL '
          ...
          mensaje = 'N§ORDENÄF.INGRESOÄÄMODELOÄÄÄÄSERIEÄÄÄÄÄÄÄREPTO.ÄÄÄMOB.ÄÄFLETEÄTALLÄTIPO'
     ELSE
          campo = 'subst(codfabo,1,1)+numdoc+" "+dtoc(fecemi)+" "+substr(codmod,1,09)+" "+str(solrep,10,2)+" "+str(solmob,10,2)+" "+str(solfle,10,2)+" "+codtall+" "+indori'
          mensaje = 'N§ORDENÄF.INGRESOÄÄMODELOÄÄÄÄÄÄÄÄREPTO.ÄÄÄÄÄÄM.O.BÄÄÄÄÄÄFLETEÄTALLÄTIPO'
     ENDIF
     DO esc_indica WITH 1, 'AYU',  ;
        'TOD', 'BBB', 'SEL'
