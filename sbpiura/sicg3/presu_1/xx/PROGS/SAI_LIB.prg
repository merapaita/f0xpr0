     ...
     IF LEN(ALLTRIM(mvalor)) = 6  ;
        ...
        8
          ...
          v3 = val_para(SUBSTR(mvalor,  ;
               3, 2),'MODAPL', ;
               'C')
          v4 = val_para(SUBSTR(mvalor,  ;
               5, 2),'ESPGAS', ;
               'C')
          v5 = IIF(LEN(ALLTRIM(mvalor)) =  ;
               8,  ;
               val_para1(SUBSTR(mvalor,  ;
               7, 2),'SUBESP' +  ;
               SUBSTR(mvalor, 5,  ;
               2),'C'), .T.)
          IF v1 .AND. v2 .AND. v3  ;
...
ENDIF
mvalor = itepar.codpart
mdescr = IIF(LEN(ALLTRIM(mvalor)) =  ;
         8,  ;
         val_para1(SUBSTR(mvalor,  ;
         7, 2),'SUBESP' +  ;
         SUBSTR(mvalor, 5, 2), ;
         'D'),  ;
         val_para(SUBSTR(mvalor,  ;
         5, 2),'ESPGAS','D'))
IF  .NOT. EMPTY(malias)
     SELECT (malias)
ENDIF
...
@ 2, 21 GET xxx PICTURE  ;
  '!!!!!!!!!!' COLOR ,N 
READ
xxx = CHRTRAN(UPPER(ALLTRIM(xxx)), 'ABCDEFGHIJKLMN¥OPQRSTUVWXYZ0123456789',;
...
ENDIF
IF LASTKEY() <> 27
     vencrip = CHRTRAN(UPPER(pass_str),;
...
DO CASE
     ...
     CASE nivel = '7'
          ccampos = 'CatAsi.TipPre+CatAsi.Generic+CatAsi.SGN1+CatAsi.SGN2+CatAsi.EspN1+CatAsi.EspN2+CatAsi.EspN3'
          lfor = "CatAsi.TipPre+CatAsi.Generic+CatAsi.Sgn1+CatAsi.Sgn2+CatAsi.EspN1+CatAsi.EspN2 =              '" +  ;
                 ...
                 itepar.espn1 +  ;
                 itepar.espn2 +  ;
                 "' AND !EMPTY(CatAsi.EspN3)"
     CASE nivel = '8'
          ccampos = 'CatAsi.TipPre+CatAsi.Generic+CatAsi.SGN1+CatAsi.SGN2+CatAsi.EspN1+CatAsi.EspN2+CatAsi.EspN3+CatAsi.EspN4'
          lfor = "CatAsi.TipPre+CatAsi.Generic+CatAsi.Sgn1+CatAsi.Sgn2+CatAsi.EspN1+CatAsi.EspN2+CatAsi.EspN3 = '" +  ;
                 itepar.tippre +  ;
                 itepar.generic +  ;
                 itepar.sgn1 +  ;
                 itepar.sgn2 +  ;
