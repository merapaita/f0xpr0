...
IF sistema = '1'
     SELECT 6
     USE
     USE IN 6 \emer97\data\itehc  ;
         ALIAS itehce ORDER  ;
         itehc1
     SELECT 3
     USE
     USE IN 3 \emer97\data\maepre  ;
         ALIAS maepree ORDER  ;
         maepre1
     SELECT itehce
     IF vtotal = 2
...
.T.);
.AND. IIF(;
.NOT. EMPTY(vcodfte), codfte = ALLTRIM(vcodfte),;
.T.);
...
INDEX ON LEFT(estfun, 5) +  ;
      LEFT(codpart, 2) TO (vind)
DO reporte WITH 2, 'SalCalr',  ;
   ' Consolidado de GENERAL ', 1,  ;
   .F., .T.
USE
ERASE (vind)
SELECT calen
