...
USE IN 5 EXCLUSIVE (fdbf) ALIAS def
SELECT def
...
ELSE
     ...
     SCAN
          ...
          ENDIF
     ENDSCAN
     DEFINE WINDOW w_fecha FROM 05, 10 TO 15, 60 TITLE vmens01 DOUBLE COLOR SCHEME 10
     ACTIVATE WINDOW w_fecha
     nfecha = 1
     @ 01, 01 SAY 'Selecciones Fecha :' COLOR SCHEME 5 GET nfecha DEFAULT afecha(1) SIZE 6, 14 FROM afecha
     READ
     ...
     GOTO TOP
     SELECT astpat
     SET FILTER TO periodo = def.periodo .AND. nummes = PADL(ALLTRIM(STR(MONTH(afecha(nfecha)))), 2, '0') .AND. numref = numpec .AND. fecha = afecha(nfecha) .AND. tipdoc = 'PEC'
     SELECT def
     DO repprg WITH 'PECOSARep', ' PE.CO.SAïs'
ENDIF
SET RELATION TO
SELECT astpat
SET FILTER TO
...
SCAN WHILE def.numpec = itepec.numpec
     ...
     DO CASE
          CASE linea < 39
               ...
               @ linea, 03 SAY CHR(179)
               @ linea, 04 SAY itepec.codart
               @ linea, 20 SAY CHR(179)
               @ linea, 21 SAY itepec.canreq
               @ linea, 30 SAY CHR(179)
               @ linea, 31 SAY LEFT(ALLTRIM(itepec.descri),50)
               @ linea, 75 SAY CHR(179)
               @ linea, 76 SAY CHR(179)
               @ linea, 78 SAY itepec.numoc
               @ linea, 83 SAY CHR(179)
               @ linea, 84 SAY itepec.candesp PICTURE '@Z 99,999.99'
               @ linea, 93 SAY CHR(179)
               @ linea, 94 SAY itepec.fecdesp
               @ linea, 104 SAY CHR(179)
               @ linea, 105 SAY itepec.coduni PICTURE '@TI'
               @ linea, 116 SAY CHR(179)
               @ linea, 128 SAY CHR(179)
